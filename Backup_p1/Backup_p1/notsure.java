...
class Player {
    ...
    private boolean readyToFightBoss = false;
    ...
    public boolean hasItem(String item) {
        return inventory.stream().anyMatch(i -> i.equalsIgnoreCase(item));
    }

    public void removeItem(String item) {
        inventory.removeIf(i -> i.equalsIgnoreCase(item));
    }
    ...
    public void setReadyToFightBoss(boolean ready) {
        this.readyToFightBoss = ready;
    }

    public boolean isReadyToFightBoss() {
        return readyToFightBoss;
    }
}
...
class World {
    ...
    private void createWorld() {
        ...
        Room lockedRoom = new Room("Locked Room", "This room is locked and contains the Final Boss.");
...
public class WorldDemo {
    public static void main(String[] args) {
        ...
        System.out.println("\nType 'left', 'right', 'forward', 'backward' to move. Type 'inventory' to view items. Type 'trade' to trade. Type 'fight' to fight. Type 'quit' to exit.");
        ...
        while (true) {
            ...
            if (command.equals("trade")) {
                if (currentPlayer.getCurrentRoom().equals(otherPlayer.getCurrentRoom())) {
                    System.out.print("Enter the item to trade: ");
                    String item = scanner.nextLine().trim();
                    if (currentPlayer.hasItem(item)) {
                        currentPlayer.removeItem(item);
                        otherPlayer.addItem(item);
                        System.out.println("Trade successful. " + item + " given to " + otherPlayer.getName());
                    } else {
                        System.out.println("You don't have that item.");
                    }
                } else {
                    System.out.println("You must be in the same room to trade.");
                }
                turn++;
                continue;
            }
            ...
            if (command.equals("fight")) {
                if (currentPlayer.getCurrentRoom().getName().equals("Locked Room") && !bossDefeated) {
                    currentPlayer.setReadyToFightBoss(true);
                    System.out.println("You are ready to fight the Final Boss. Waiting for the other player...");

                    if (otherPlayer.getCurrentRoom().equals(currentPlayer.getCurrentRoom()) && otherPlayer.isReadyToFightBoss()) {
                        System.out.println("Both players are ready. You have defeated the Final Boss together! YOU WIN!");
                        bossDefeated = true;
                    }
                } else {
            ...
            if (nextRoom != null) {
                ...
                if (nextRoom.getName().equals("Enemy Room")) {
                    ...
                    if (choice.equals("fight") && !currentPlayer.hasItem("Sword")) {
                        ...
                        if (pickKey.equals("yes")) {
                            ...
                            lockedRoom.setDescription("This room is unlocked and contains the Final Boss.");
                        }
                ...
                currentPlayer.setReadyToFightBoss(false);
