...
class Room {
    ...
    public void playerEntered(Player p) {
        notifyObservers(p.getName() + " has entered " + this.name);
    }
...
class World {
    ...
    public Room setForest_description(){
        Room forest = rooms.get("Forest");
        System.out.println("Player 1 and player 2 are now in: " + forest.getName());
        return forest;
...
public class WorldDemo {
    public static void main(String[] args) {
        ...
        //register 
        for (String roomName : new String[]{"Forest", "Green Forest", "Dark Forest", "Enemy Room", "Chest Room", "Locked Room"}) {
            Room room = world.getRoom(roomName);
            if (room != null) {
                room.addObserver(logger);
                room.addObserver(notifier);  // Register notification
            }
        }
        world.setForest_description();
        Player player1 = new Player("Player 1", world.setForest());
        Player player2 = new Player("Player 2", world.setForest());
        ...
        while (true) {
            ...
            System.out.println();
            currentPlayer.getCurrentRoom().playerEntered(currentPlayer); 
